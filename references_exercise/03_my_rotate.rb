...
def my_rotate!(array, amt)
    ...
    temp_array = [] # will temporalilly store the values rotated

    (0..array.length-1).each { |i| temp_array[i] = array[(i+amt)%array.length] }
    (0..array.length-1).each { |i| array[i] = temp_array [i] }
